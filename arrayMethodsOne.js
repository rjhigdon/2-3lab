...
const postTaxPrices = prices.map(function(element){
  return (element + (element * 0.07)).toFixed(2)
})
...
let orderTotals = orders.map(newPrice=> newPrice.price * (newPrice.tax + 1))


//I am trying to find a way to see if you need to use callbacks for this or if oyu can do it all in one line. whether oyu need to set up a function that calculates the total tax and one that adds it to the price or if you can just calculate the total plus tax all in one line easy peasy
...
// CODE HERE
//find a way to filter out everyone else's totals using the .filter method then add them together using the .reduce method. when it comes to these things in these kinds of arrays it gets tough 
